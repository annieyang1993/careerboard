...
function JobPost() {
    ...
    const [experienceBool, setExperienceBool] = useState(false);
    ...
    const CheckoutForm = () => {
        ...
        const handleSubmitPayment = async (event) => {
            ...
            if (paymentMethodReq.error){
                ...
                if (tags!==null && tags!=='' && tags!==','){
                    ...
                    await Firebase.firestore().collection('tag_filters').doc('tag_filters').set(tagHash, {merge: true})
                }
                ...
                await Firebase.firestore().collection('location_filters').doc('location_filters').set(locationHash, {merge: true})
        ...
        return (
            <form >
                ...
                <button className='next' disabled ={loadingIndicator || firebaseError || successful} onClick={(e)=>handleSubmitPayment(e)}>Purchase Posting ($20)</button>
    ...
    return (
        <div className = "content">
            ...
            <form onSubmit={(e)=>handleSubmit(e)} className="job-post-form">
                ...
                <div className = 'label-and-entry'>
                <label className = "title" for="company-description">Tags/Skills:</label>
                <div className = 'entry-container'>
                <input className = "entry" placeholder = 'Comma separated (e.g. React, Rust, Swift, Non-Tech, DeFi, Mobile).' type="text" id="company-description" name="company-description" value={tags} onChange={(event)=>setTags(event.target.value)}/>
                </div>
                </div>

                {experienceBool === false && submitClicked === true ? 
                <div className = 'label-and-entry' >
                ...
                <div className = 'entry-container'>
                <input className = "entry" type="number" id="company-description" placeholder = 'Minimum annual salary (e.g. 100000). Optional but recommended to increase views.' name="company-description" value={compensationLow === 0 || compensationLow === '' ? null : compensationLow} onChange={(event)=>setCompensationLow(event.target.value)}/>
                </div>
                </div>
                ...
                <div className = 'entry-container'>
                <input className = "entry" type="number" id="company-description" name="company-description" placeholder = 'Maximum annual salary (e.g. 100000). Optional but recommended to increase views.' value={compensationHigh === 0 || compensationLow === '' ? null : compensationHigh} onChange={(event)=>setCompensationHigh(event.target.value)}/>
                </div>
                </div>
            ...
            {screen===1 ? 
             ...
             <form onSubmit={(e)=>handleSubmit1(e)} className="job-post-form">
                <div className = 'job-header-info'>
                    ...
                    <div className = 'job-page-compensation'>
                        Posted On:
                        <div className = 'estimate'>{new Date().toLocaleDateString()}</div>
                    </div>



                        <input className="next" type="submit" value="Proceed to checkout ($20)"></input>
